<template>
  <div class="login-wrap">
    <el-form :model="userform"  class="login-container">
      ...
      <el-form-item >
        用户名：<el-input type="text" placeholder="用户账号" v-model="userform.name" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item>
        密码：<el-input type="password" placeholder="用户密码" v-model="userform.password" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary"  @click="check"  style="width: 100%;">登录</el-button>
      </el-form-item>
...
<script>
  export default {
    ...
    methods: {
		async check(){    
			let res = this.$axios.post('/login',this.userform,JSON.stringify({
		...
		let request = await res
		console.log(request.data)
		if (request.code === 200){
			 window.localStorage.setItem('token',request.data[0].token)
			this.$router.push('/buju/home')
				}
				else{
					this.$message.error('账号或者用户名错误，请重新输入')
					this.userform = {}
				}
		},
		toRegister(){
